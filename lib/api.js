...
function fetchAllCallback(data) {
  ...
  request.addEventListener("load", function () {
    if (request.status === 200){
      ...
      if (typeof(loginList) === "object") {
         loginList = Object.keys(loginList).map(function (key) { return loginList[key]; });
      }
      ...
      for (var i=0; i<loginList.length; i++) {
        if (loginList[i].hasOwnProperty("id") &&
            loginList[i]["id"] !== 0 &&
            (loginList[i]["deleted"] === "0" || loginList[i]["deleted"] === false)) {
          var obj = jsonToObject(loginList[i]);
          if (obj !== null) {
            tempLoginList.push(obj);
          }
        }
      }
