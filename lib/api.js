...
function create(databaseHost, databaseUser, databasePassword, loginname, pass, website, address, notes, callback) {
  var encodedLogin = base64.encode(databaseUser + ":" + databasePassword);
  var request = new xhr.XMLHttpRequest({"mozAnon": true});
  var data = {
    "website": website,
...
function update(databaseHost, databaseUser, databasePassword, id, loginname, pass, website, address, notes, datechanged, deleted, callback) {
  var encodedLogin = base64.encode(databaseUser + ":" + databasePassword);
  var request = new xhr.XMLHttpRequest({"mozAnon": true});
  var data = {
    "website": website,
  ...
  if (callback != undefined) {
    apiRequest.addEventListener("load", callback);
  }
